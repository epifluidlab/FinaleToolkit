...
<<<<<<< Updated upstream
    ...
    parser_command3.add_argument('-p', '--intersect_policy', default='midpoint', type=str, help='Specifies what policy is used to include fragments in the given interval. See User Guide for more information.')
...
>>>>>>> Stashed changes
    ...
    parser_command3_1.add_argument('-p', '--intersect_policy', default='midpoint', type=str, help='Specifies what policy is used to include fragments in the given interval. See User Guide for more information.')
    ...
    parser_command5 = subparsers.add_parser('delfi', prog='finaletoolkit-delfi', description='Calculates DELFI score over genome. NOTE: Due to some ad hoc implementation details, currently the only accepted reference genome is hg19.')
    ...
    parser_command5.add_argument('-o', '--output_file', default='-', help='BED, bed.gz, TSV, or CSV file to write results to. If "-", writes to stdout.')
    ...
    parser_command9.add_argument('input_file', help='BED3+3 file containing raw data')
    parser_command9.add_argument('-o', '--output-file', default='-', help='BED3+3 to print GC-corrected DELFI fractions. If "-", will write to stdout. Default is "-".')
    parser_command9.add_argument('--header-lines', default=1, type=int, help='Number of header lines in BED. Default is 1.')
    parser_command9.add_argument('-v', '--verbose', action='count', help='Specify verbosity. Number of printed statements is proportional to number of vs.')
    parser_command9.set_defaults(func=cli_delfi_gc_correct)

    parser_command10 = subparsers.add_parser('end-motifs', prog='finaletoolkit-end-motifs', description="Measures frequency of k-mer 5' end motifs and tabulates data into a tab-delimited file.")
    parser_command10.add_argument('input_file', help='SAM, BAM, or tabix-indexed file with fragment data.')
    parser_command10.add_argument('refseq_file', help='2bit file containing reference sequence that fragments were aligned to.')
    parser_command10.add_argument('-k', default=4, type=int, help='Length of k-mer. Default is 4.')
    parser_command10.add_argument('-o', '--output-file', default='-', help='TSV to print k-mer frequencies. If "-", will write to stdout. Default is "-".')
    parser_command10.add_argument('-q', '--quality-threshold', default=20, type=int, help='Minimum MAPQ of reads. Default is 20.')
    parser_command10.add_argument('-w', '--workers', default=1, type=int, help='Number of subprocesses to use. Default is 1.')
    parser_command10.add_argument('-v', '--verbose', default=0, action='count', help='Specify verbosity. Number of printed statements is proportional to number of vs.')
    ...
    parser_command10a.add_argument('input_file', help='SAM, BAM, or tabix-indexed file with fragment data.')
    parser_command10a.add_argument('refseq_file', help='2bit file containing reference sequence that fragments were aligned to.')
    parser_command10a.add_argument('intervals', help='BED file containing intervals or list of tuples')
    parser_command10a.add_argument('-k', default=4, type=int, help='Length of k-mer. Default is 4.')
    parser_command10a.add_argument('-lo', '--fraction-low', default=10, type=int, help='Smallest fragment length to consider. Default is 10')
    parser_command10a.add_argument('-hi', '--fraction-high', default=600, type=int, help='Longest fragment length to consider. Default is 600')
    parser_command10a.add_argument('-o', '--output-file', default='-', help="File path to write results to. Either tsv or csv.")
    parser_command10a.add_argument('-q', '--quality-threshold', default=20, type=int, help='Minimum MAPQ of reads. Default is 20.')
    parser_command10a.add_argument('-w', '--workers', default=1, type=int, help='Number of subprocesses to use. Default is 1.')
    parser_command10a.add_argument('-v', '--verbose', default=0, action='count', help='Specify verbosity. Number of printed statements is proportional to number of vs.')
    parser_command10a.set_defaults(func=interval_end_motifs)

    parser_command11 = subparsers.add_parser('mds', prog='finaletoolkit-mds', description='Reads k-mer frequencies from a file and calculates a motif diversity score (MDS) using normalized Shannon entropy as described by Jiang et al (2020). This function is generalized for any k-mer instead of just 4-mers.')
    ...
    parser_command11.add_argument('-s', '--sep', default='\t', help='Separator used in tabular file. Default is tab.')
    ...
    parser_command11a = subparsers.add_parser('interval-mds', prog='finaletoolkit-interval-mds', description='Reads k-mer frequencies from a file and calculates a motif diversity score (MDS) for each interval using normalized Shannon entropy as described by Jiang et al (2020). This function is generalized for any k-mer instead of just 4-mers.')
    ...
    parser_command11a.add_argument('-s', '--sep', default='\t', help='Separator used in tabular file. Default is tab.')
    parser_command11a.add_argument('file_out', default='-')
